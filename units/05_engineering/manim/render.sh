#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# render.sh — Render Manim scenes for Unit 05: Physics & Engineering.
#
# Usage:
#   ./render.sh                         # render all three scenes
#   ./render.sh total_internal_reflection    # render a specific scene
#   ./render.sh transformer
#   ./render.sh motor_effect
#
# Quality:
...
#
# Output lands in units/05_engineering/manim/output/ (gitignored).
# ──────────────────────────────────────────────────────────────────────
set -euo pipefail
...
for scene in "${TARGETS[@]}"; do
    # Container path = host path relative to REPO_ROOT, under /work
    SCENE_FILE_CONTAINER="/work/units/05_engineering/manim/scenes/${scene}.py"

    echo ""
    echo "═══════════════════════════════════════════════════════════"
    echo "  Rendering:  ${scene}"
    echo "  Quality:    ${QUALITY}"
    echo "  Container:  ${SCENE_FILE_CONTAINER}"
    echo "═══════════════════════════════════════════════════════════"
    echo ""
    ...
    docker run --rm \
        ...
        "$IMAGE" \
        manim \
            "$SCENE_FILE_CONTAINER" \
            "$QUALITY" \
            --disable_caching \
            --format mp4 \
            --media_dir "/work/units/05_engineering/manim/output"
