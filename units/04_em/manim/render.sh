#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# render.sh — Render Manim scenes for Unit 04: Electricity & Magnetism.
#
# Usage:
#   ./render.sh                         # render all three scenes
#   ./render.sh electric_field_lines    # render a specific scene
#   ./render.sh potential_gradient
#   ./render.sh circuit_comparison
#
# Quality:
...
#
# Output lands in units/04_em/manim/output/ (gitignored).
# ──────────────────────────────────────────────────────────────────────
set -euo pipefail
...
SCENE_NAMES=("electric_field_lines" "potential_gradient" "circuit_comparison")
...
for scene in "${TARGETS[@]}"; do
    ...
    docker run --rm \
        ...
        manim \
            ...
            --disable_caching \
            --format mp4 \
            --media_dir "/work/units/04_em/manim/output"
