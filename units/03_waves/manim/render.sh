#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# render.sh — Render Manim scenes for Unit 03: Waves.
#
# Usage:
#   ./render.sh                         # render all three scenes
#   ./render.sh superposition_standing  # render a specific scene
#   ./render.sh wave_speed_intensity
#   ./render.sh young_slit
#
# Quality:
...
#
# Output lands in units/03_waves/manim/output/ (gitignored).
# ──────────────────────────────────────────────────────────────────────
set -euo pipefail
...
for scene in "${TARGETS[@]}"; do
    # Container-relative path (repo root is mounted at /work)
    CONTAINER_SCENE_FILE="/work/units/03_waves/manim/scenes/${scene}.py"

    echo ""
    echo "═══════════════════════════════════════════════════════════"
    echo "  Rendering:  ${scene}"
    echo "  Quality:    ${QUALITY}"
    echo "  File:       ${CONTAINER_SCENE_FILE}"
    echo "═══════════════════════════════════════════════════════════"
    echo ""
    ...
    docker run --rm \
        ...
        "$IMAGE" \
        manim \
            "$CONTAINER_SCENE_FILE" \
            "$QUALITY" \
            --disable_caching \
            --format mp4 \
            --media_dir "/work/units/03_waves/manim/output"
